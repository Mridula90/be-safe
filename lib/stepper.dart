...
class _Stepper2State extends State<StepperTouch>
  ...
  void initState() {
    ...
    if (widget.direction == Axis.horizontal) {
      ...
      _animation = Tween<Offset>(begin: Offset(0.0, 0.0), end: Offset(0.0, 4.5))
          .animate(_controller);
    }
  }
  ...
  Widget build(BuildContext context) {
    return FittedBox(
      child: Container(
        width: widget.direction == Axis.horizontal ? 280.0 : 60.0,
        height: widget.direction == Axis.horizontal ? 120.0 : 180.0,
        child: Material(
          type: MaterialType.canvas,
          clipBehavior: Clip.antiAlias,
          borderRadius: BorderRadius.circular(60.0),
          color: Color(4278217470).withOpacity(0.6),
          child: Stack(
            alignment: Alignment.topCenter,
            children: <Widget>[
              Positioned(
                left: widget.direction == Axis.horizontal ? 10.0 : null,
                bottom: widget.direction == Axis.horizontal ? null : 13.0,
                child: RotatedBox(
                  quarterTurns: 5,
                  child: Text(
                    "",
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    if (_controller.value <= -0.20) {
      setState(() => isHor ? widget.onSlide : _value++);
      changed = true;
    } else if (_controller.value >= 0.20) {
      setState(() => isHor ? _value++ : widget.onSlide);
      changed = true;
    }
